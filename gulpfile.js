...
const sass = require('gulp-sass'); // НАЧИНАЯ с 5-й версии Gulp, вместо строки 12 использовать строку 13.
...
const ghPages = require('gulp-gh-pages'); // РАСКОММЕНТИРОВАТЬ ЭТУ СТРОКУ для Создания Сайта на gh-pages и "Деплоя" на этот сайт моего проекта.
...
// 1. B Пустую папку с Новым проектом из Папки gulp-start перенести ВСЕ 5 файлов!!! (gulpfile.js, package.json, index.html, style.css и папку src);

// 2. B консоли VSCode открыть Папку с Новым проектом, написать и запустить команду npm install (установятся все нужные модули);

// 3. После того, как всё корректно установится и в папке с Новым проектом появится папка node_modules и файл package-lock.json, нужно в консоли ввести и запустить команду gulp (после её отработки появится Приветственная страница в браузере). После этого нужно ОБЯЗАТЕЛЬНО удалить из этой папки с Новым проектом Пустые файлы index.html и style.css (которые в самом начале перенесли из Папки my-gulp-start в эту папку с Новым проектом) и из папки src/assets/scss Удалить папку _style2.scss !!! и в папке main.scss удалить строчку с кодом @import "./style2" !!!;

// 4. После этого нужно Подключить этот новый проект к Git по методичке Урока №1.

// 5. Соблюдаем файловую структуру или в сборках подправляем пути "откуда берем"/"куда кладем" файлы.

// 6. !!!!! Для Деплоя готового проекта на GitHub-pages РАСКОММЕНТИРОВАТЬ строку 23 ЭТОГО файла Gulpfile.js, затем в консоли VSCode ввести сначала команду npm install --save-dev gulp-gh-pages после её отработки ввести команду gulp deploy   !!!!!

// 7. После того, как Деплой без ошибок завершён, в консоли VSCode написать и поочерёдно запустить команды: git add . и git commit -m"addition""  и git push !!!